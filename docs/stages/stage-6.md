...
## Domain currency, approach validity and quality review

| RACI | Role | Responsibility |
|------|------|----------------|
| **A** | Informatics Training Lead (for computational & data elements) / Lab Training Lead (for lab elements) | Accountable — they are the domain authority for their area. Their sign-off is the quality gate. This review covers: whether the approaches, tools and methods being taught are current and still meaningful in the field; whether they are appropriate for this learner level; and where an older approach is retained, whether there is a clear educational rationale supporting its inclusion. If something is no longer current practice without justification, the Lead is accountable for flagging it. |
| **R** | Education Developer(s) | Responsible for collating materials and managing the review process. |
| **C** | Subject Matter Experts | Consulted where the Training Lead wants a second opinion on whether a specific approach or method remains current in active research. |
| **I** | Head of LDD | Informed that sign-off has been given. |

!!! info "What this review is — and isn't"
    This is a curatorial and quality review, not a fact-check. The Training Lead is asking: are we teaching the right things? Are these approaches current, meaningful, and appropriate for this audience? If a newer tool or workflow has superseded what's in the course, the Lead flags it. If an older approach is retained for sound educational reasons — for example, as a conceptual foundation before introducing current methods — the Lead confirms that reasoning is valid and it is documented.
...
| RACI | Role | Responsibility |
|------|------|----------------|
| **A/R** | Education Developer(s) | Accountable for the quality of the overall instructional design — learning outcomes, curriculum structure, content organisation, and assessment approach where assessment is included. This applies across all delivery formats. |
...
=== "Training Lead reviews for: approach currency, validity and quality"

    **Informatics Training Lead checks:**

    - Are the computational tools, data types and analytical approaches current and still meaningful in the field — or have they been superseded by newer methods?
    - Is the tool or pipeline choice appropriate for this audience's level and learning goals — not too advanced, not too simplistic?
    - If an older approach is included, is there a clear educational reason for it (e.g. as a conceptual foundation), and is that reason documented?
    - Are the expected outputs what the analysis would actually produce with current tooling?
    - Is the level of computational complexity appropriate for the learner group?

    **Lab Training Lead checks:**

    - Are the experimental methods and protocols methodologically sound and representative of current practice in the field?
    - Is the technique appropriate for this audience and their level of prior knowledge?
    - If a technique is no longer standard practice, is there a documented educational rationale for including it?
